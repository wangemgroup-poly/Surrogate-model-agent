...
**适用范围。** 只要优化目标能由一维结果算出，任何 CST 参数化频域工程都能驱动：滤波器、耦合器、
匹配网络、波导器件、频率选择表面、天线。方法本身与天线无关，唯一与天线绑定的是远场旁瓣指标
（`sll_phi_cut`）。如果用户的目标需要现有指标类型算不出来的量，直说，不要凑一个近似的。
...
如果用户还没定好目标，或者你不确定这个工程能不能用，就把向导交给用户，而不是自己去写配置。
它是交互式的，**必须在用户能敲键盘的终端里跑**，不要放在你自己的工具调用里：

```bat
.venv\Scripts\python.exe agent.py setup --lang zh
```

它会体检工程、找出同结构的其他版本、建议参数范围、列出可用的结果树、把频带端点对齐到采样点、
给出分段建议，并检查数据量够不够。它不会启动求解器。用户希望你代劳时，再按下面的步骤自己做。
...
"全带取最差值"这种目标，只在最差点大体待在同一位置时才好建模。一旦最差点在设计之间跳来跳去
（通常是谐振点在带内移动），这个量就不再是参数的光滑函数，替代模型在它上面的误差会超过正在追求的
改进量，闭环就会卡在整带同一个平台上不动。
...
`worst_frequencies` 给出每个设计的最差点频率；如果它们聚成两三簇且彼此分开，`suggest_segments`
会给出切分点，`segmentation_gain` 则在用户自己的设计上对整带与分段两种写法做交叉验证，返回两个误差。
建议分段时要把这两个数一起报出来——分段会改变配置签名，代价是重新训练；而在真正平坦的频带上它毫无收益。
每个子频带写成独立的指标项、各自给门限；单一门限表达不了的指标形状，也是这样提出来的。
